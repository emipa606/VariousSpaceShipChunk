...
namespace ShipChunkDrop_Transpiler
{
    ...
    internal static class HarmonyPatches
    {
        ...
        {
            if (LoadedModManager.GetMod<ShipChunkDrop_TranspilerMod>().GetSettings<ShipChunkDrop_TranspilerSettings>().SmallChunksAlso)
            {
            ...
            return (from defs in DefDatabase<ThingDef>.AllDefs
                    where defs.defName.StartsWith("ShipChunk") && !defs.defName.Contains("Incoming") && !defs.defName.Contains("small")
                    select defs).RandomElement();
        }
    }
}
